...
class PaymentController extends Controller {
    ...
    public function store(Group $group)
    {
        ...
        if (\request()->how == 'item' && !(\request()->item_select ?? null)) {

            throw ValidationException::withMessages(['how_much' => 'You owe no items to this user']);
        }
        if ($attributes['how_much'] > abs($user->getBalanceWithUser($user2, $group)) && !($attributes['item_select'] ?? null)) {
            throw ValidationException::withMessages(['how_much' => 'Too large payment amount']);
        }

        if ($attributes['item_select'] ?? null)
            $item_max = abs($user->getItemBalanceWithUser($user2, $group)[$attributes['item_select']]);
        if (($attributes['item_select'] ?? null) && $attributes['how_much'] > $item_max) {

            throw ValidationException::withMessages(['how_much' => 'Too large payment amount for this item']);
        }
